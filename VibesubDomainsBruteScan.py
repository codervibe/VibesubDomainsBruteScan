...
async def main():
    ...
    scan_count = 0
    found_count = 0
    queue_size_array = [0] * options.process
        ...
        with ProcessPoolExecutor(max_workers=options.process) as executor:
            tasks = [
                loop.run_in_executor(
                    ...
                    (domain, options, process_num, dns_servers, next_subs, scan_count, found_count, queue_size_array, tmp_dir)
                ) for process_num in range(options.process)
            ]

            for count, task in enumerate(asyncio.as_completed(tasks)):
                await task
                groups_count = sum(queue_size_array)
                msg = f'[{"/-\\|"[count % 4]}] {found_count} found, {scan_count} scanned in {time.time() - start_time:.1f} seconds, {groups_count} groups left'
                logging.info(msg)
    ...
    msg = f'All Done. {domain_count} found, {scan_count} scanned in {time.time() - start_time:.1f} seconds.'
    logging.info(msg)
